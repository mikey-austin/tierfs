...
// ── Shared UI ──────────────────────────────────────────────────────────────────
function TierBadge({tier}){const i=+tier.replace("tier","")||0;const c=T.tierColors[i%T.tierColors.length];return <span style={{fontFamily:"'IBM Plex Mono',monospace",fontSize:26,fontWeight:600,background:c+"22",color:c,border:`1px solid ${c}44`,borderRadius:2,padding:"1px 6px",whiteSpace:"nowrap"}}>{tier}</span>;}
function StateBadge({state}){const m={synced:[T.green],local:[T.amber],syncing:[T.cyan],writing:[T.blue]};const[c]=m[state]||[T.muted];return <span style={{fontFamily:"'IBM Plex Mono',monospace",fontSize:26,fontWeight:600,background:c+"22",color:c,border:`1px solid ${c}44`,borderRadius:2,padding:"1px 6px",whiteSpace:"nowrap"}}>{state.toUpperCase()}</span>;}
function LvlBadge({level}){const m={info:T.muted,warn:T.amber,error:T.red,debug:T.blue};return <span style={{fontFamily:"'IBM Plex Mono',monospace",fontSize:26,fontWeight:600,color:m[level]||T.muted,minWidth:34,display:"inline-block"}}>{level.toUpperCase()}</span>;}
function Chip({label,color=T.muted}){return <span style={{fontFamily:"'Barlow Condensed',sans-serif",fontSize:26,fontWeight:600,letterSpacing:"0.08em",background:color+"22",color,border:`1px solid ${color}44`,borderRadius:2,padding:"1px 7px",whiteSpace:"nowrap"}}>{label}</span>;}
function SHdr({children}){return <div style={{fontFamily:"'Barlow Condensed',sans-serif",fontSize:26,fontWeight:700,letterSpacing:"0.2em",color:T.muted,textTransform:"uppercase",margin:"16px 0 10px",display:"flex",alignItems:"center",gap:8}}><div style={{flex:1,height:1,background:T.border}}/>{children}<div style={{flex:4,height:1,background:T.border}}/></div>;}
function SC({label,value,sub,color}){return <div style={{background:T.card,border:`1px solid ${T.border}`,borderRadius:3,padding:"11px 13px",minWidth:0}}><div style={{fontFamily:"'Barlow Condensed',sans-serif",fontSize:22,fontWeight:600,letterSpacing:"0.15em",color:T.muted,textTransform:"uppercase",marginBottom:4}}>{label}</div><div style={{fontFamily:"'IBM Plex Mono',monospace",fontSize:24,fontWeight:500,color:color||T.text,lineHeight:1}}>{value}</div>{sub&&<div style={{fontFamily:"'IBM Plex Mono',monospace",fontSize:26,color:T.muted,marginTop:3}}>{sub}</div>}</div>;}
function Sel({value,onChange,children}){return <select value={value} onChange={e=>onChange(e.target.value)} style={{background:T.card,border:`1px solid ${T.border2}`,borderRadius:3,color:T.text,fontFamily:"'IBM Plex Mono',monospace",fontSize:21,padding:"5px 8px",outline:"none",cursor:"pointer"}}>{children}</select>;}
...
function Loading(){return <div style={{fontFamily:"'IBM Plex Mono',monospace",fontSize:21,color:T.muted,padding:20,textAlign:"center"}}>Loading…</div>;}
    ...
    <div>
      ...
      <div style={{background:T.card,border:`1px solid ${T.border}`,borderRadius:3,overflow:"hidden"}}>
        <div style={{padding:"9px 14px",background:T.surface,borderBottom:`1px solid ${T.border}`,display:"flex",flexWrap:"wrap",gap:14,alignItems:"center"}}>
          <span style={{fontFamily:"'Barlow Condensed',sans-serif",fontSize:26,fontWeight:700,letterSpacing:"0.15em",color:T.muted,textTransform:"uppercase"}}>{tierDefs.map(t=>t.name).join(" → ")}</span>
          <span style={{flex:1}}/>
          <span style={{fontFamily:"'IBM Plex Mono',monospace",fontSize:21,color:T.cyan}}>repl: {fmtRate(last.replBytesS||0)}</span>
          <span style={{fontFamily:"'IBM Plex Mono',monospace",fontSize:21,color:T.amber}}>queue: {last.replQueueDepth||0}</span>
        </div>
            ...
            {tiers.map((t,i)=>{
              ...
              return(
                <g key={t.name} onMouseEnter={()=>setHov(t.name)} onMouseLeave={()=>setHov(null)} style={{cursor:"default"}}>
                  ...
                  <text x={t.x+10} y={t.y+57} fill={T.text} fontSize={22} fontFamily="'IBM Plex Mono',monospace" fontWeight="500">{t.count.toLocaleString()}</text>
                  <text x={t.x+10} y={t.y+68} fill={T.muted} fontSize={11} fontFamily="'Barlow Condensed',sans-serif" letterSpacing="0.08em">FILES</text>
                  <text x={t.x+10} y={t.y+83} fill={T.muted} fontSize={16} fontFamily="'IBM Plex Mono',monospace">{fmtB(t.used)}</text>
        ...
        <div style={{margin:"0 14px 14px",background:T.surface,border:`1px solid ${T.border}`,borderRadius:3,overflowX:"auto"}}>
          <div style={{display:"grid",gridTemplateColumns:"100px 130px 90px 1fr 90px",minWidth:480}}>
            {["Rule","Match","Pin Tier","Evict Schedule","Promote"].map(h=>(
              <div key={h} style={{padding:"6px 10px",background:T.bg,fontFamily:"'Barlow Condensed',sans-serif",fontSize:22,fontWeight:700,letterSpacing:"0.12em",color:T.muted,textTransform:"uppercase",borderBottom:`1px solid ${T.border}`}}>{h}</div>
            ))}
            {rules.map((r,i)=>[
              <div key="n" style={{padding:"6px 10px",fontFamily:"'IBM Plex Mono',monospace",fontSize:21,color:T.text,background:i%2===0?T.card:"transparent",borderBottom:i<rules.length-1?`1px solid ${T.border}`:"none"}}>{r.name}</div>,
              <div key="m" style={{padding:"6px 10px",fontFamily:"'IBM Plex Mono',monospace",fontSize:26,color:T.muted,background:i%2===0?T.card:"transparent",borderBottom:i<rules.length-1?`1px solid ${T.border}`:"none"}}>{r.match}</div>,
              <div key="p" style={{padding:"5px 10px",background:i%2===0?T.card:"transparent",borderBottom:i<rules.length-1?`1px solid ${T.border}`:"none",display:"flex",alignItems:"center"}}>{r.pinTier?<TierBadge tier={r.pinTier}/>:<span style={{color:T.border2,fontSize:21,fontFamily:"'IBM Plex Mono',monospace"}}>—</span>}</div>,
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(200px,1fr))",gap:7,marginBottom:14}}>
        {[{label:"Write (MiB/s)",key:"backendWriteMBs",color:T.amber},{label:"Queue Depth",key:"replQueueDepth",color:T.cyan},{label:"FUSE Ops/s",key:"fuseOpRate",color:T.purple}].map(({label,key,color})=>(
          <div key={key} style={{background:T.card,border:`1px solid ${T.border}`,borderRadius:3,padding:"10px 12px"}}>
            <div style={{fontFamily:"'Barlow Condensed',sans-serif",fontSize:22,fontWeight:700,letterSpacing:"0.15em",color:T.muted,textTransform:"uppercase",marginBottom:2}}>{label}</div>
            <div style={{fontFamily:"'IBM Plex Mono',monospace",fontSize:21,color,marginBottom:6}}>{key==="replQueueDepth"?(last[key]||0):(last[key]||0).toFixed(1)}</div>
        ...
        {tierDefs.map((t,i)=>{const used=usage[t.name]||0,pct=t.capacity?Math.min(1,used/t.capacity):0,c=pct>0.85?T.red:pct>0.7?T.amber:T.tierColors[i%T.tierColors.length];return(
          <div key={t.name} style={{marginBottom:10}}>
            <div style={{display:"flex",justifyContent:"space-between",marginBottom:3,flexWrap:"wrap",gap:4}}>
              <span style={{display:"flex",alignItems:"center",gap:7}}><span style={{width:6,height:6,borderRadius:"50%",background:T.tierColors[i%T.tierColors.length],display:"inline-block"}}/><span style={{fontFamily:"'IBM Plex Mono',monospace",fontSize:22,color:T.text}}>{t.name}</span><span style={{fontFamily:"'Barlow Condensed',sans-serif",fontSize:26,color:T.muted}}>{t.backend}</span></span>
              <span style={{fontFamily:"'IBM Plex Mono',monospace",fontSize:21,color:c}}>{t.capacity?`${fmtB(used)} / ${fmtB(t.capacity)}`:fmtB(used)}</span>
            </div>
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(110px,1fr))",gap:7}}>
        {[["synced",T.green],["local",T.amber],["syncing",T.cyan],["writing",T.blue]].map(([s,c])=>(
          <div key={s} style={{background:T.card,border:`1px solid ${T.border}`,borderRadius:3,padding:"10px 12px"}}>
            <div style={{fontFamily:"'Barlow Condensed',sans-serif",fontSize:22,fontWeight:700,letterSpacing:"0.15em",color:T.muted,textTransform:"uppercase"}}>{s}</div>
            <div style={{fontFamily:"'IBM Plex Mono',monospace",fontSize:26,color:c,marginTop:3}}>{states[s]||0}</div>
  ...
  return(
    <div>
      ...
      <div style={{display:"flex",gap:6,marginBottom:8,flexWrap:"wrap"}}>
        <input value={search} onChange={e=>{setSearch(e.target.value);setPg(0);}} placeholder="Search paths…" style={{background:T.card,border:`1px solid ${T.border2}`,borderRadius:3,color:T.text,fontFamily:"'IBM Plex Mono',monospace",fontSize:21,padding:"5px 10px",flex:"1 1 140px",minWidth:0,outline:"none"}}/>
      ...
      <div style={{fontFamily:"'IBM Plex Mono',monospace",fontSize:26,color:T.muted,marginBottom:5}}>{filtered.length.toLocaleString()} files · page {pg+1}/{tp}</div>
      <div style={{background:T.card,border:`1px solid ${T.border}`,borderRadius:3,overflowX:"auto"}}>
        <div style={{display:"grid",gridTemplateColumns:"2fr 70px 90px 80px 100px",minWidth:460}}>
          {[["relPath","Path"],["size","Size"],["currentTier","Tier"],["state","State"],["modTime","Modified"]].map(([k,l])=>(
            <div key={k} onClick={()=>hs(k)} style={{padding:"6px 10px",background:T.surface,fontFamily:"'Barlow Condensed',sans-serif",fontSize:22,fontWeight:700,letterSpacing:"0.1em",color:sk===k?T.amber:T.muted,textTransform:"uppercase",cursor:"pointer",userSelect:"none",borderBottom:`1px solid ${T.border}`,whiteSpace:"nowrap"}}>{l}{sk===k?(sd==="asc"?" ↑":" ↓"):""}</div>
          ))}
          {pd.map((f,i)=>[
            <div key="p" style={{padding:"5px 10px",fontFamily:"'IBM Plex Mono',monospace",fontSize:26,color:T.text,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap",background:i%2===0?T.card:T.surface+"88",borderBottom:`1px solid ${T.border}`}} title={f.relPath}>{f.relPath}</div>,
            <div key="s" style={{padding:"5px 10px",fontFamily:"'IBM Plex Mono',monospace",fontSize:26,color:T.muted,background:i%2===0?T.card:T.surface+"88",borderBottom:`1px solid ${T.border}`}}>{fmtB(f.size)}</div>,
  ...
  return(
    <div>
      ...
      <div style={{background:T.card,border:`1px solid ${T.border}`,borderRadius:3,padding:"10px 12px",marginBottom:12}}>
        <div style={{fontFamily:"'Barlow Condensed',sans-serif",fontSize:22,fontWeight:700,letterSpacing:"0.15em",color:T.muted,marginBottom:5,textTransform:"uppercase"}}>Throughput (bytes/s)</div>
        <ResponsiveContainer width="100%" height={100}>
          <AreaChart data={metrics.slice(-40)} margin={{top:4,right:4,left:0,bottom:0}}>
            ...
            <XAxis dataKey="ts" tick={{fontFamily:"'IBM Plex Mono',monospace",fontSize:21,fill:T.muted}} interval={9}/>
            <YAxis tickFormatter={v=>fmtB(v)} tick={{fontFamily:"'IBM Plex Mono',monospace",fontSize:21,fill:T.muted}} width={54}/>
      ...
      <div style={{background:T.card,border:`1px solid ${T.border}`,borderRadius:3,overflowX:"auto"}}>
        <div style={{display:"grid",gridTemplateColumns:"1fr 140px 50px 90px",minWidth:400}}>
          {["Path","Tiers","Retry","Enqueued"].map(h=><div key={h} style={{padding:"6px 10px",background:T.surface,fontFamily:"'Barlow Condensed',sans-serif",fontSize:22,fontWeight:700,letterSpacing:"0.1em",color:T.muted,textTransform:"uppercase",borderBottom:`1px solid ${T.border}`,whiteSpace:"nowrap"}}>{h}</div>)}
          {queue.map((j,i)=>[
            <div key="p" style={{padding:"5px 10px",fontFamily:"'IBM Plex Mono',monospace",fontSize:26,color:T.text,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap",background:i%2===0?T.card:"transparent",borderBottom:`1px solid ${T.border}`}} title={j.relPath}>{j.relPath}</div>,
            ...
            <div key="r" style={{padding:"5px 10px",fontFamily:"'IBM Plex Mono',monospace",fontSize:26,color:j.retries>0?T.amber:T.muted,background:i%2===0?T.card:"transparent",borderBottom:`1px solid ${T.border}`}}>{j.retries}</div>,
            <div key="e" style={{padding:"5px 10px",fontFamily:"'IBM Plex Mono',monospace",fontSize:26,color:T.muted,background:i%2===0?T.card:"transparent",borderBottom:`1px solid ${T.border}`}}>{j.enqueuedAt?fmtRel(j.enqueuedAt):"—"}</div>,
          ])}
          {queue.length===0&&<div style={{gridColumn:"1/-1",padding:"18px",textAlign:"center",fontFamily:"'IBM Plex Mono',monospace",fontSize:21,color:T.muted}}>Queue empty</div>}
        </div>
      </div>
    </div>
  );
    ...
    <div>
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(200px,1fr))",gap:7}}>
        {ch.map(({label,key,color,fmt})=>(
          <div key={key} style={{background:T.card,border:`1px solid ${T.border}`,borderRadius:3,padding:"10px 12px"}}>
            <div style={{fontFamily:"'Barlow Condensed',sans-serif",fontSize:22,fontWeight:700,letterSpacing:"0.15em",color:T.muted,textTransform:"uppercase",marginBottom:2}}>{label}</div>
            <div style={{fontFamily:"'IBM Plex Mono',monospace",fontSize:26,color,marginBottom:7}}>{fmt(last[key]||0)}</div>
  ...
  return(
    <div>
      ...
      <div style={{background:T.card,border:`1px solid ${T.amber}33`,borderRadius:3,padding:"12px 14px",marginBottom:12}}>
        <div style={{fontFamily:"'Barlow Condensed',sans-serif",fontSize:22,fontWeight:700,letterSpacing:"0.15em",color:T.amber,marginBottom:7,textTransform:"uppercase"}}>Active Write Handles</div>
        {wg.length===0?<div style={{fontFamily:"'IBM Plex Mono',monospace",fontSize:21,color:T.muted}}>No open handles</div>:wg.map((e,i)=>(
          <div key={i} style={{display:"flex",alignItems:"center",gap:10,padding:"7px 0",borderBottom:i<wg.length-1?`1px solid ${T.border}`:"none",flexWrap:"wrap"}}>
            ...
            <div style={{fontFamily:"'IBM Plex Mono',monospace",fontSize:21,color:T.text,flex:"1 1 180px",minWidth:0,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{e.relPath}</div>
            <span style={{fontFamily:"'IBM Plex Mono',monospace",fontSize:26,color:T.muted}}>×{e.openCount}</span>
      ...
      <div style={{background:T.card,border:`1px solid ${T.border}`,borderRadius:3,overflowX:"auto"}}>
        <div style={{display:"grid",gridTemplateColumns:"1fr 90px 90px 100px",minWidth:380}}>
          {["Path","Tier","State","Modified"].map(h=><div key={h} style={{padding:"6px 10px",background:T.surface,fontFamily:"'Barlow Condensed',sans-serif",fontSize:22,fontWeight:700,letterSpacing:"0.1em",color:T.muted,textTransform:"uppercase",borderBottom:`1px solid ${T.border}`}}>{h}</div>)}
          {awaitingRepl.map((f,i)=>[
            <div key="p" style={{padding:"5px 10px",fontFamily:"'IBM Plex Mono',monospace",fontSize:26,color:T.text,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap",background:i%2===0?T.card:"transparent",borderBottom:`1px solid ${T.border}`}} title={f.relPath}>{f.relPath}</div>,
            ...
            <div key="m" style={{padding:"5px 10px",fontFamily:"'IBM Plex Mono',monospace",fontSize:26,color:T.muted,background:i%2===0?T.card:"transparent",borderBottom:`1px solid ${T.border}`}}>{fmtRel(f.modTime)}</div>,
          ])}
          {awaitingRepl.length===0&&<div style={{gridColumn:"1/-1",padding:"18px",textAlign:"center",fontFamily:"'IBM Plex Mono',monospace",fontSize:21,color:T.muted}}>No files awaiting replication</div>}
        </div>
      </div>
    </div>
  );
...
function LogsView(){
  ...
  return(
    <div>
      ...
      <div style={{display:"flex",gap:6,marginBottom:8,flexWrap:"wrap"}}>
        <input value={filter} onChange={e=>setFilter(e.target.value)} placeholder="Filter…" style={{background:T.card,border:`1px solid ${T.border2}`,borderRadius:3,color:T.text,fontFamily:"'IBM Plex Mono',monospace",fontSize:21,padding:"5px 10px",flex:"1 1 140px",minWidth:0,outline:"none"}}/>
        ...
        <button onClick={()=>setFollow(f=>!f)} style={{background:follow?T.cyan+"22":T.card,border:`1px solid ${follow?T.cyan:T.border}`,borderRadius:3,color:follow?T.cyan:T.muted,fontFamily:"'Barlow Condensed',sans-serif",fontSize:26,fontWeight:700,letterSpacing:"0.08em",padding:"5px 12px",cursor:"pointer",textTransform:"uppercase",whiteSpace:"nowrap"}}>{follow?"● LIVE":"○ PAUSED"}</button>
      </div>
      <div style={{background:T.surface,border:`1px solid ${T.border}`,borderRadius:3,height:"min(440px,55vh)",overflowY:"auto"}}>
        {filtered.map((l,i)=>(
          <div key={i} style={{display:"flex",gap:0,padding:"3px 10px",background:l.level==="error"?T.red+"0D":l.level==="warn"?T.amber+"0A":"transparent",borderBottom:`1px solid ${T.border}`,overflow:"hidden"}}>
            ...
            <span style={{color:T.text,flexShrink:0,fontFamily:"'IBM Plex Mono',monospace",fontSize:26,whiteSpace:"nowrap"}}>{l.msg}</span>
            {Object.keys(l.fields||{}).length>0&&<span style={{color:T.muted,marginLeft:6,fontFamily:"'IBM Plex Mono',monospace",fontSize:26,whiteSpace:"nowrap",flexShrink:0}}>{Object.entries(l.fields).map(([k,v])=>`${k}=${v}`).join(" ")}</span>}
          </div>
        ))}
        <div ref={botRef}/>
      </div>
    ...
    <div>
          ...
          return(
            <div key={t.name} style={{background:T.card,border:`1px solid ${c}44`,borderRadius:3,padding:"13px 15px",position:"relative",overflow:"hidden"}}>
              ...
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:9}}>
                <div><div style={{fontFamily:"'IBM Plex Mono',monospace",fontSize:22,fontWeight:500,color:c}}>{t.name}</div><div style={{fontFamily:"'Barlow Condensed',sans-serif",fontSize:26,color:T.muted,marginTop:1}}>P{t.priority} · {t.scheme}</div></div>
                <Chip label="OK" color={T.green}/>
              </div>
              <div style={{fontFamily:"'IBM Plex Mono',monospace",fontSize:22,color:T.muted,marginBottom:7,wordBreak:"break-all"}}>{be?.uri}</div>
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:5,marginBottom:9}}>
                <div><div style={{fontFamily:"'Barlow Condensed',sans-serif",fontSize:21,color:T.muted,letterSpacing:"0.08em",textTransform:"uppercase"}}>Files</div><div style={{fontFamily:"'IBM Plex Mono',monospace",fontSize:26,color:T.text}}>{count.toLocaleString()}</div></div>
                <div><div style={{fontFamily:"'Barlow Condensed',sans-serif",fontSize:21,color:T.muted,letterSpacing:"0.08em",textTransform:"uppercase"}}>Used</div><div style={{fontFamily:"'IBM Plex Mono',monospace",fontSize:26,color:T.text}}>{fmtB(used)}</div></div>
              </div>
              {t.capacity&&<><div style={{height:4,background:T.border,borderRadius:2,overflow:"hidden",marginBottom:3}}><div style={{height:"100%",width:`${(pct*100).toFixed(0)}%`,background:bc,borderRadius:2}}/></div><div style={{fontFamily:"'IBM Plex Mono',monospace",fontSize:22,color:T.muted}}>{(pct*100).toFixed(1)}% of {fmtB(t.capacity)}</div></>}
      ...
      <div style={{background:T.card,border:`1px solid ${T.border}`,borderRadius:3,overflowX:"auto"}}>
        <div style={{display:"grid",gridTemplateColumns:"100px 55px 1fr 70px",minWidth:360}}>
          {["Name","Type","URI","Status"].map(h=><div key={h} style={{padding:"6px 10px",background:T.surface,fontFamily:"'Barlow Condensed',sans-serif",fontSize:22,fontWeight:700,letterSpacing:"0.1em",color:T.muted,textTransform:"uppercase",borderBottom:`1px solid ${T.border}`}}>{h}</div>)}
          {backends.map((b,i)=>[
            <div key="n" style={{padding:"6px 10px",fontFamily:"'IBM Plex Mono',monospace",fontSize:21,color:T.text,background:i%2===0?T.card:"transparent",borderBottom:`1px solid ${T.border}`}}>{b.name}</div>,
            <div key="t" style={{padding:"6px 10px",fontFamily:"'IBM Plex Mono',monospace",fontSize:26,color:T.muted,background:i%2===0?T.card:"transparent",borderBottom:`1px solid ${T.border}`}}>{b.type}</div>,
            <div key="u" style={{padding:"6px 10px",fontFamily:"'IBM Plex Mono',monospace",fontSize:22,color:T.muted,background:i%2===0?T.card:"transparent",borderBottom:`1px solid ${T.border}`,wordBreak:"break-all"}}>{b.uri}</div>,
  ...
  const SidebarContent=(
    <div style={{width:"100%",height:"100%",background:T.surface,borderRight:`1px solid ${T.border}`,display:"flex",flexDirection:"column"}}>
      <div style={{padding:collapsed?"12px 0":"12px 14px",borderBottom:`1px solid ${T.border}`,display:"flex",alignItems:"center",justifyContent:collapsed?"center":"space-between",gap:8,flexShrink:0}}>
        {!collapsed&&<div><div style={{fontFamily:"'Barlow Condensed',sans-serif",fontSize:21,fontWeight:700,letterSpacing:"0.1em",color:T.amber}}>TIER<span style={{color:T.text}}>FS</span></div><div style={{fontFamily:"'Barlow Condensed',sans-serif",fontSize:21,fontWeight:600,letterSpacing:"0.2em",color:T.muted}}>ADMIN</div></div>}
        <button onClick={()=>setCollapsed(c=>!c)}
          title={collapsed?"Expand sidebar":"Collapse sidebar"}
          style={{background:"transparent",border:`1px solid ${T.border}`,borderRadius:2,color:T.muted,width:24,height:24,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",fontSize:21,flexShrink:0,lineHeight:1}}>
          {collapsed?"›":"‹"}
        </button>
      </div>
      {/* Health */}
      <div style={{padding:collapsed?"7px 0":"7px 12px",borderBottom:`1px solid ${T.border}`,display:"flex",alignItems:"center",justifyContent:collapsed?"center":"flex-start",gap:6,flexShrink:0}}>
        ...
        {!collapsed&&<span style={{fontFamily:"'Barlow Condensed',sans-serif",fontSize:22,fontWeight:700,letterSpacing:"0.1em",color:apiOk?T.green:T.red}}>{apiOk?"CONNECTED":"API ERROR"}</span>}
      </div>
      {/* Nav */}
      <nav style={{padding:"6px 0",flex:1,overflowY:"auto"}}>
        {NAV.map(({id,label,icon})=>{
          ...
          return(
            ...
            >
              <span style={{fontSize:26,color:active?T.amber:T.muted,lineHeight:1,flexShrink:0}}>{icon}</span>
              {!collapsed&&<span style={{fontFamily:"'Barlow Condensed',sans-serif",fontSize:26,fontWeight:active?600:400,letterSpacing:"0.06em",color:active?T.amber:T.muted}}>{label}</span>}
            </div>
          );
        })}
      </nav>
      {/* Footer */}
      {!collapsed
        ?<div style={{padding:"9px 12px",borderTop:`1px solid ${T.border}`,flexShrink:0,fontFamily:"'IBM Plex Mono',monospace",fontSize:21,color:T.muted,lineHeight:1.8}}>v0.1.0 · {tierDefs.length} tiers<br/><span style={{color:tick%2===0?T.cyan:T.muted}}>● live</span></div>
        ...
        <div style={{flex:1,display:"flex",flexDirection:"column",overflow:"hidden",minWidth:0}}>
          ...
          <div style={{background:T.surface,borderBottom:`1px solid ${T.border}`,padding:"0 12px",height:44,display:"flex",alignItems:"center",gap:8,flexShrink:0}}>
            <button onClick={()=>setMobileOpen(true)}
              style={{background:"transparent",border:`1px solid ${T.border}`,borderRadius:2,color:T.muted,width:26,height:26,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",fontSize:22,flexShrink:0}}
              aria-label="Open menu">☰</button>

            <div style={{fontFamily:"'Barlow Condensed',sans-serif",fontSize:26,fontWeight:600,letterSpacing:"0.1em",color:T.text,flexShrink:0,minWidth:0,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>
            ...
            {[
              ...
              <div key={i} className={narrow?"hide-narrow":""} style={{fontFamily:"'IBM Plex Mono',monospace",fontSize:26,color:c,borderLeft:`1px solid ${T.border}`,paddingLeft:9,marginLeft:1,flexShrink:0,whiteSpace:"nowrap"}}>{v}</div>
            ))}
          </div>
