...
// ── Static data ───────────────────────────────────────────────────────────────
const TIER_DEFS=[
  {name:"tier0",backend:"ssd",      scheme:"file://",capacity:536870912000, priority:0,transforms:[]},
  {name:"tier1",backend:"nas-smb",  scheme:"smb://", capacity:8796093022208,priority:1,transforms:["zstd"]},
  {name:"tier2",backend:"minio",    scheme:"s3://",  capacity:null,         priority:2,transforms:["zstd","aes-256-gcm"]},
  {name:"tier3",backend:"backblaze",scheme:"s3://",  capacity:null,         priority:3,transforms:["aes-256-gcm"]},
];
const BACKENDS=[
  {name:"ssd",       uri:"file:///data/ssd/cctv",       type:"file",status:"ok"},
  {name:"nas-smb",   uri:"smb://nas.lan/recordings",     type:"smb", status:"ok"},
  {name:"minio",     uri:"s3://nvr-archive/cctv",        type:"s3",  status:"ok"},
  {name:"backblaze", uri:"s3://my-nvr-bucket/cold-cctv", type:"s3",  status:"ok"},
];
const RULES=[
  {name:"exports",   match:"exports/**",   pinTier:"tier0",evictSchedule:[],promoteOnRead:false},
  {name:"clips",     match:"clips/**",     pinTier:"",evictSchedule:[{after:"168h",to:"tier1"}],promoteOnRead:"tier0"},
  {name:"recordings",match:"recordings/**",pinTier:"",evictSchedule:[{after:"24h",to:"tier1"},{after:"720h",to:"tier2"},{after:"8760h",to:"tier3"}],promoteOnRead:false},
  {name:"thumbnails",match:"thumbnails/**",pinTier:"",evictSchedule:[{after:"1h",to:"tier1"}],promoteOnRead:false},
  {name:"default",   match:"**",           pinTier:"",evictSchedule:[{after:"48h",to:"tier1"}],promoteOnRead:false},
];

// ── Sim data generators ────────────────────────────────────────────────────────
function makeFiles(n=300){
  const cams=["cam1","cam2","cam3","cam4","cam_door","cam_garden"];
  const pfx=["recordings","clips","thumbnails","exports"];
  const states=["synced","local","syncing","writing"];
  const sw=[0.7,0.15,0.1,0.05];
  const now=Date.now();
  return Array.from({length:n},(_,i)=>{
    const p=pfx[Math.floor(Math.random()*pfx.length)];
    const cam=cams[i%cams.length];
    const d=new Date(now-Math.random()*7*86400*1000);
    const age=(now-d.getTime())/3600000;
    const tier=age<24?"tier0":age<720?"tier1":age<8760?"tier2":"tier3";
    let r=Math.random(),si=0,acc=0;
    for(let j=0;j<sw.length;j++){acc+=sw[j];if(r<acc){si=j;break;}}
    return{id:i,relPath:`${p}/${cam}/${d.toISOString().slice(0,10)}/${d.toTimeString().slice(0,8).replace(/:/g,"-")}.${p==="thumbnails"?"jpg":"mp4"}`,currentTier:tier,state:states[si],size:p==="thumbnails"?50000+Math.random()*200000:5e6+Math.random()*500e6,modTime:d.toISOString(),digest:Math.random()>0.1?"xxh3:"+Math.random().toString(36).slice(2,18):""};
  });
}
function makeJobs(n=12){
  const tt=["tier0","tier1","tier2","tier3"];
  return Array.from({length:n},(_,i)=>{const fi=Math.floor(Math.random()*3);return{id:`job-${i}`,relPath:`recordings/cam${1+i%4}/2026-03-14/10-${String(i*5%60).padStart(2,"0")}-00.mp4`,fromTier:tt[fi],toTier:tt[fi+1],retries:Math.floor(Math.random()*3),size:10e6+Math.random()*500e6,enqueuedAt:new Date(Date.now()-Math.random()*600000).toISOString()};});
}
function makeMetrics(n=60){
  return Array.from({length:n},(_,i)=>{const t=new Date(Date.now()-i*5000);return{ts:t.toTimeString().slice(0,8),backendWriteMBs:20+Math.random()*80,backendReadMBs:5+Math.random()*30,replBytesS:1e6+Math.random()*5e6,replQueueDepth:Math.floor(Math.random()*50),fuseOpRate:50+Math.random()*200,fuseLatP99Ms:0.5+Math.random()*5,metaLatP99Ms:0.1+Math.random()*2,evictionRate:Math.random()*3};}).reverse();
}
function makeLogs(n=80){
  const ll=["info","info","info","info","warn","error"];
  const lg=["tier-service","tier-service.replicator","tier-service.evictor","fuse","smb","sftp","s3","transform"];
  const ms=[
    {msg:"copying file",fields:{rel_path:"recordings/cam1/2026-03-14/10-00-00.mp4",from:"file:///...",to:"smb://nas.lan/..."}},
    {msg:"evicted file to next tier",fields:{from:"tier0",to:"tier1"}},
    {msg:"SFTP backend connected",fields:{tier:"tier1",host:"nas.lan:22"}},
    {msg:"backend transform pipeline resolved",fields:{pipeline:"zstd → aes-256-gcm"}},
    {msg:"promoting file for write",fields:{from:"tier2",to:"tier0"}},
    {msg:"delaying replication: write-active",fields:{reason:"quiescence window"}},
    {msg:"file finalised and purged",fields:{final_tier:"tier3"}},
    {msg:"digest verification failed",fields:{chunk:"4"}},
    {msg:"SMB reconnecting",fields:{error:"connection reset"}},
    {msg:"staleness check: modified since enqueue",fields:{path:"clips/cam3/new.mp4"}},
  ];
  const now=Date.now();
  return Array.from({length:n},(_,i)=>{const m=ms[Math.floor(Math.random()*ms.length)];return{ts:new Date(now-(n-i)*1200-Math.random()*800).toISOString(),level:ll[Math.floor(Math.random()*ll.length)],logger:lg[Math.floor(Math.random()*lg.length)],...m};});
}
...
// ── Shared UI ──────────────────────────────────────────────────────────────────
function TierBadge({tier}){const i=+tier.replace("tier","")||0;const c=T.tierColors[i];return <span style={{fontFamily:"'IBM Plex Mono',monospace",fontSize:10,fontWeight:600,background:c+"22",color:c,border:`1px solid ${c}44`,borderRadius:2,padding:"1px 6px",whiteSpace:"nowrap"}}>{tier}</span>;}
...
function LvlBadge({level}){const m={info:T.muted,warn:T.amber,error:T.red};return <span style={{fontFamily:"'IBM Plex Mono',monospace",fontSize:10,fontWeight:600,color:m[level]||T.muted,minWidth:34,display:"inline-block"}}>{level.toUpperCase()}</span>;}
...
// ── TOPOLOGY VIEW ─────────────────────────────────────────────────────────────
function TopologyView({files,metrics}){
  const[hov,setHov]=useState(null);
  const[anim,setAnim]=useState(0);
  useEffect(()=>{const t=setInterval(()=>setAnim(a=>(a+1)%100),60);return()=>clearInterval(t);},[]);

  const usage=useMemo(()=>{const u={};TIER_DEFS.forEach(t=>u[t.name]=0);files.forEach(f=>{if(f.currentTier in u)u[f.currentTier]+=f.size;});return u;},[files]);
  ...
  const totalW=TIER_DEFS.length*(TW+GAP)-GAP;
  const startX=(W-totalW)/2;
  const TY=(H-TH)/2+10;

  const tiers=TIER_DEFS.map((t,i)=>({
    ...t,x:startX+i*(TW+GAP),y:TY,
    color:T.tierColors[i],
    used:usage[t.name]||0,
    count:cnt[t.name]||0,
    pct:t.capacity?Math.min(1,(usage[t.name]||0)/t.capacity):0,
    backend:BACKENDS.find(b=>b.name===t.backend),
  }));

  // Eviction schedule lines
  const evictLines=RULES.flatMap(r=>r.evictSchedule.map(s=>{
    const fi=tiers.findIndex(t=>t.name===r.name||true);// always show all connections
    const from=tiers.find(t=>r.match.includes(t.name)||true);
    const fromT=tiers[0];// writes always start tier0
    const toT=tiers.find(t=>t.name===s.to);
    if(!toT)return null;
    const fromIdx=tiers.findIndex(t=>t.name!==s.to&&tiers.indexOf(t)<tiers.indexOf(toT));
    if(fromIdx<0)return null;
    return null; // handled below with simple sequential arrows
  })).filter(Boolean);

  // Simple sequential flow arrows
    ...
    <div>
      ...
      <div style={{background:T.card,border:`1px solid ${T.border}`,borderRadius:3,overflow:"hidden"}}>
        ...
        <div style={{padding:"9px 14px",background:T.surface,borderBottom:`1px solid ${T.border}`,display:"flex",flexWrap:"wrap",gap:14,alignItems:"center"}}>
          <span style={{fontFamily:"'Barlow Condensed',sans-serif",fontSize:10,fontWeight:700,letterSpacing:"0.15em",color:T.muted,textTransform:"uppercase"}}>tier0 → tier1 → tier2 → tier3</span>
          <span style={{flex:1}}/>
                ...
                <g key={t.name} onMouseEnter={()=>setHov(t.name)} onMouseLeave={()=>setHov(null)} style={{cursor:"default"}}>
                  ...
                  {t.transforms.length===0
                    ...
                    :t.transforms.map((tr,ti)=>{
                      const tc=tr==="aes-256-gcm"?T.green:tr==="zstd"?T.purple:T.muted;
                      return(<g key={tr}>
                  ...
                  <circle cx={t.x+TW-10} cy={t.y+TH-10} r={4}
                    fill={t.backend?.status==="ok"?T.green:T.red}
                    style={{filter:`drop-shadow(0 0 4px ${t.backend?.status==="ok"?T.green:T.red})`}}/>
                </g>
              );
            })}
        ...
        <div style={{margin:"0 14px 14px",background:T.surface,border:`1px solid ${T.border}`,borderRadius:3,overflowX:"auto"}}>
          <div style={{display:"grid",gridTemplateColumns:"100px 130px 90px 1fr 90px",minWidth:480}}>
            ...
            {RULES.map((r,i)=>[
              ...
              <div key="pr" style={{padding:"5px 10px",background:i%2===0?T.card:"transparent",borderBottom:i<RULES.length-1?`1px solid ${T.border}`:"none",display:"flex",alignItems:"center"}}>{r.promoteOnRead?<TierBadge tier={typeof r.promoteOnRead==="string"?r.promoteOnRead:"tier0"}/>:<span style={{color:T.border2,fontSize:10,fontFamily:"'IBM Plex Mono',monospace"}}>—</span>}</div>,
            ])}
          </div>
        </div>
      </div>
...
// ── DASHBOARD ─────────────────────────────────────────────────────────────────
function DashboardView({files,metrics,queue,wg}){
  const usage=useMemo(()=>{const u={};TIER_DEFS.forEach(t=>u[t.name]=0);files.forEach(f=>{if(f.currentTier in u)u[f.currentTier]+=f.size;});return u;},[files]);
  ...
  return(
    <div>
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(130px,1fr))",gap:7,marginBottom:14}}>
        ...
        <SC label="Write"       value={`${(last.backendWriteMBs||0).toFixed(0)}M/s`} sub="tier0" color={T.amber}/>
        <SC label="Repl"        value={fmtRate(last.replBytesS||0)}  sub="tier0→tier1" color={T.cyan}/>
      ...
      <div style={{background:T.card,border:`1px solid ${T.border}`,borderRadius:3,padding:"12px 14px",marginBottom:14}}>
        {TIER_DEFS.map((t,i)=>{const used=usage[t.name]||0,pct=t.capacity?Math.min(1,used/t.capacity):0,c=pct>0.85?T.red:pct>0.7?T.amber:T.tierColors[i];return(
          <div key={t.name} style={{marginBottom:10}}>
            <div style={{display:"flex",justifyContent:"space-between",marginBottom:3,flexWrap:"wrap",gap:4}}>
              <span style={{display:"flex",alignItems:"center",gap:7}}><span style={{width:6,height:6,borderRadius:"50%",background:T.tierColors[i],display:"inline-block"}}/><span style={{fontFamily:"'IBM Plex Mono',monospace",fontSize:12,color:T.text}}>{t.name}</span><span style={{fontFamily:"'Barlow Condensed',sans-serif",fontSize:10,color:T.muted}}>{t.backend}</span></span>
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(110px,1fr))",gap:7}}>
        {[["synced",T.green],["local",T.amber],["syncing",T.cyan],["writing",T.blue]].map(([s,c])=>(
          <div key={s} style={{background:T.card,border:`1px solid ${T.border}`,borderRadius:3,padding:"10px 12px"}}>
            ...
            <div style={{height:3,background:T.border,borderRadius:2,marginTop:5,overflow:"hidden"}}><div style={{height:"100%",width:`${((states[s]||0)/files.length*100).toFixed(0)}%`,background:c,borderRadius:2}}/></div>
          </div>
        ))}
      </div>
    </div>
...
function FilesView({files}){
  ...
  const filtered=useMemo(()=>{let f=files;if(search)f=f.filter(x=>x.relPath.toLowerCase().includes(search.toLowerCase()));if(ft!=="all")f=f.filter(x=>x.currentTier===ft);if(fs!=="all")f=f.filter(x=>x.state===fs);f=[...f].sort((a,b)=>{let av=a[sk],bv=b[sk];if(sk==="modTime"){av=new Date(av);bv=new Date(bv);}return sd==="asc"?(av>bv?1:-1):(av<bv?1:-1);});return f;},[files,search,ft,fs,sk,sd]);
  ...
  return(
    <div>
      ...
      <div style={{display:"flex",gap:6,marginBottom:8,flexWrap:"wrap"}}>
        ...
        <Sel value={ft} onChange={v=>{setFt(v);setPg(0);}}><option value="all">All tiers</option>{["tier0","tier1","tier2","tier3"].map(t=><option key={t} value={t}>{t}</option>)}</Sel>
...
// ── REPLICATION ───────────────────────────────────────────────────────────────
function ReplicationView({queue,metrics}){
  const last=metrics[metrics.length-1]||{};
  return(
    <div>
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(130px,1fr))",gap:7,marginBottom:12}}>
        ...
        <SC label="Throughput" value={fmtRate(last.replBytesS||0)} sub="tier0→tier1" color={T.cyan}/>
        <SC label="Error Rate" value="0.0%"                    sub="5m window" color={T.green}/>
      </div>
      ...
      <div style={{background:T.card,border:`1px solid ${T.border}`,borderRadius:3,overflowX:"auto"}}>
        <div style={{display:"grid",gridTemplateColumns:"80px 1fr 140px 70px 50px 90px",minWidth:500}}>
          {["Job","Path","Tiers","Size","Retry","Enqueued"].map(h=><div key={h} style={{padding:"6px 10px",background:T.surface,fontFamily:"'Barlow Condensed',sans-serif",fontSize:9,fontWeight:700,letterSpacing:"0.1em",color:T.muted,textTransform:"uppercase",borderBottom:`1px solid ${T.border}`,whiteSpace:"nowrap"}}>{h}</div>)}
          {queue.map((j,i)=>[
            <div key="id" style={{padding:"5px 10px",fontFamily:"'IBM Plex Mono',monospace",fontSize:10,color:T.muted,background:i%2===0?T.card:"transparent",borderBottom:`1px solid ${T.border}`}}>{j.id}</div>,
            ...
            <div key="s" style={{padding:"5px 10px",fontFamily:"'IBM Plex Mono',monospace",fontSize:10,color:T.muted,background:i%2===0?T.card:"transparent",borderBottom:`1px solid ${T.border}`}}>{fmtB(j.size)}</div>,
            ...
            <div key="e" style={{padding:"5px 10px",fontFamily:"'IBM Plex Mono',monospace",fontSize:10,color:T.muted,background:i%2===0?T.card:"transparent",borderBottom:`1px solid ${T.border}`}}>{fmtRel(j.enqueuedAt)}</div>,
          ])}
...
function PerformanceView({metrics}){
  const ch=[{label:"Write (MiB/s)",key:"backendWriteMBs",color:T.amber,fmt:v=>`${v.toFixed(1)} MiB/s`},{label:"Read (MiB/s)",key:"backendReadMBs",color:T.cyan,fmt:v=>`${v.toFixed(1)} MiB/s`},{label:"FUSE Ops/s",key:"fuseOpRate",color:T.purple,fmt:v=>`${v.toFixed(0)}/s`},{label:"FUSE p99 ms",key:"fuseLatP99Ms",color:T.blue,fmt:v=>`${v.toFixed(2)}ms`},{label:"Meta p99 ms",key:"metaLatP99Ms",color:T.green,fmt:v=>`${v.toFixed(2)}ms`},{label:"Eviction/hr",key:"evictionRate",color:T.red,fmt:v=>`${v.toFixed(1)}/hr`}];
  const last=metrics[metrics.length-1]||{};
  return(
    <div>
...
// ── WRITE GUARD ───────────────────────────────────────────────────────────────
function WriteGuardView({wg,files}){
  const recent=useMemo(()=>files.filter(f=>f.state==="writing"||f.state==="local").slice(0,20),[files]);
  return(
    <div>
      ...
      <div style={{background:T.card,border:`1px solid ${T.amber}33`,borderRadius:3,padding:"12px 14px",marginBottom:12}}>
        ...
        {wg.length===0?<div style={{fontFamily:"'IBM Plex Mono',monospace",fontSize:11,color:T.muted}}>No open handles</div>:wg.map((e,i)=>(
          <div key={i} style={{display:"flex",alignItems:"center",gap:10,padding:"7px 0",borderBottom:i<wg.length-1?`1px solid ${T.border}`:"none",flexWrap:"wrap"}}>
            ...
            <Chip label="WRITING" color={T.cyan}/>
          </div>
        ))}
      </div>
...
function LogsView({logs}){
  ...
  useEffect(()=>{if(follow&&botRef.current)botRef.current.scrollIntoView({behavior:"smooth"});},[logs,follow]);
  const filtered=useMemo(()=>{let l=logs;if(lv!=="all")l=l.filter(x=>x.level===lv);if(filter)l=l.filter(x=>x.msg.includes(filter)||x.logger.includes(filter));return l;},[logs,lv,filter]);
  return(
    <div>
      ...
      <div style={{display:"flex",gap:6,marginBottom:8,flexWrap:"wrap"}}>
        ...
        <Sel value={lv} onChange={setLv}><option value="all">All</option><option value="info">INFO</option><option value="warn">WARN</option><option value="error">ERROR</option></Sel>
    ...
    <div>
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(195px,1fr))",gap:9,marginBottom:18}}>
        {TIER_DEFS.map((t,i)=>{
          ...
          const c=T.tierColors[i];const bc=pct>0.85?T.red:pct>0.7?T.amber:c;
          return(
            <div key={t.name} style={{background:T.card,border:`1px solid ${c}44`,borderRadius:3,padding:"13px 15px",position:"relative",overflow:"hidden"}}>
              ...
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:9}}>
                ...
                <Chip label={be?.status==="ok"?"OK":"DOWN"} color={be?.status==="ok"?T.green:T.red}/>
              </div>
              ...
              {t.transforms?.length>0&&<div style={{marginTop:7,display:"flex",gap:4,flexWrap:"wrap"}}>{t.transforms.map(tr=><Chip key={tr} label={tr} color={tr==="aes-256-gcm"?T.green:T.purple}/>)}</div>}
            </div>
          );
        })}
      </div>
      ...
      <div style={{background:T.card,border:`1px solid ${T.border}`,borderRadius:3,overflowX:"auto"}}>
        <div style={{display:"grid",gridTemplateColumns:"100px 55px 1fr 70px",minWidth:360}}>
          ...
          {BACKENDS.map((b,i)=>[
            ...
            <div key="s" style={{padding:"5px 10px",background:i%2===0?T.card:"transparent",borderBottom:`1px solid ${T.border}`,display:"flex",alignItems:"center"}}><Chip label={b.status.toUpperCase()} color={b.status==="ok"?T.green:T.red}/></div>,
          ])}
        </div>
      </div>
    </div>
...
export default function TierFSAdmin(){
  ...
  const[files]=useState(()=>makeFiles(350));
  const[metrics,setMetrics]=useState(()=>makeMetrics(60));
  const[queue]=useState(()=>makeJobs(12));
  const[wg]=useState(()=>[
    {relPath:"recordings/cam1/2026-03-14/11-32-00.mp4",openCount:1},
    {relPath:"recordings/cam3/2026-03-14/11-32-05.mp4",openCount:1},
  ]);
  const[logs,setLogs]=useState(()=>makeLogs(80));
  ...
  useEffect(()=>{
    const t=setInterval(()=>{
      ...
      setMetrics(m=>{
        ...
        return[...m.slice(-119),{ts:new Date().toTimeString().slice(0,8),backendWriteMBs:cl(last.backendWriteMBs+(Math.random()-0.48)*8,0,200),backendReadMBs:cl(last.backendReadMBs+(Math.random()-0.5)*4,0,100),replBytesS:cl(last.replBytesS+(Math.random()-0.5)*5e5,0,1e7),replQueueDepth:cl(last.replQueueDepth+Math.floor((Math.random()-0.55)*4),0,100),fuseOpRate:cl(last.fuseOpRate+(Math.random()-0.5)*20,0,500),fuseLatP99Ms:cl(last.fuseLatP99Ms+(Math.random()-0.5)*0.5,0.1,20),metaLatP99Ms:cl(last.metaLatP99Ms+(Math.random()-0.5)*0.2,0.05,10),evictionRate:cl(last.evictionRate+(Math.random()-0.5)*0.3,0,10)}];
      });
      if(Math.random()<0.4){setLogs(l=>{const nl=makeLogs(1)[0];nl.ts=new Date().toISOString();return[...l.slice(-199),nl];});}
    },2500);
  ...
  const SidebarContent=(
    <div style={{width:"100%",height:"100%",background:T.surface,borderRight:`1px solid ${T.border}`,display:"flex",flexDirection:"column"}}>
      ...
      <div style={{padding:collapsed?"7px 0":"7px 12px",borderBottom:`1px solid ${T.border}`,display:"flex",alignItems:"center",justifyContent:collapsed?"center":"flex-start",gap:6,flexShrink:0}}>
        ...
        {!collapsed&&<span style={{fontFamily:"'Barlow Condensed',sans-serif",fontSize:9,fontWeight:700,letterSpacing:"0.1em",color:T.green}}>ALL SYSTEMS OK</span>}
      </div>
      {/* Nav */}
      ...
      {/* Footer */}
      {!collapsed
        ?<div style={{padding:"9px 12px",borderTop:`1px solid ${T.border}`,flexShrink:0,fontFamily:"'IBM Plex Mono',monospace",fontSize:8,color:T.muted,lineHeight:1.8}}>v0.1.0 · 4 tiers<br/><span style={{color:tick%2===0?T.cyan:T.muted}}>● live</span></div>
      ...
      <div style={{display:"flex",height:"100dvh",background:T.bg,color:T.text,fontFamily:"'IBM Plex Mono',monospace",overflow:"hidden"}}>
        ...
        <div style={{flex:1,display:"flex",flexDirection:"column",overflow:"hidden",minWidth:0}}>
          ...
          <div style={{background:T.surface,borderBottom:`1px solid ${T.border}`,padding:"0 12px",height:44,display:"flex",alignItems:"center",gap:8,flexShrink:0}}>
            {/* Hamburger — visible on mobile only (hidden on desktop via media query below) */}
            ...
            {[
              ...
              [`p99:${(last.fuseLatP99Ms||0).toFixed(1)}ms`,T.muted,true],
            ].map(([v,c,narrow],i)=>(
          ...
          <div style={{flex:1,overflow:"auto",padding:"12px 14px",WebkitOverflowScrolling:"touch"}}>
            {view==="dashboard"   &&<DashboardView   files={files} metrics={metrics} queue={queue} wg={wg}/>}
            {view==="topology"    &&<TopologyView     files={files} metrics={metrics}/>}
            {view==="tiers"       &&<TiersView        files={files}/>}
            {view==="files"       &&<FilesView        files={files}/>}
            {view==="replication" &&<ReplicationView  queue={queue} metrics={metrics}/>}
            {view==="performance" &&<PerformanceView  metrics={metrics}/>}
            {view==="writeguard"  &&<WriteGuardView   wg={wg} files={files}/>}
            {view==="logs"        &&<LogsView         logs={logs}/>}
          </div>
        </div>
      </div>

      {/* Media query to hide desktop sidebar on mobile and show hamburger */}
      <style>{`
